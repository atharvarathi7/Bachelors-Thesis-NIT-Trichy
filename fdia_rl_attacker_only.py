# -*- coding: utf-8 -*-
"""FDIA_RL_Attacker_Only.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZzNL00lfA5r7-cV_NI2ZmiwAob17ofdd
"""
...
class PredictedPAttackEnv(gym.Env):
    """
    Environment for False Data Injection Attack (FDIA) on predicted P values.
    The agent directly manipulates predicted power measurements while keeping
    the total power consumption constant.
    """

    def __init__(self, initial_p_values, target_indices=None, episode_length=100):
        super(PredictedPAttackEnv, self).__init__()
    ...
    def _calculate_reward(self):
        ...
        max_allowed_deviation = 0.20 * original_targets
        ...
        attack_success = (np.all(detection_closeness <= 1.0) and
        ...
        # Record if attack was successful (changed values without exceeding detection threshold)
    ...
    def step(self, action):
      ...
      return new_observation, reward, done, {"current_p_values": self.current_p_values.copy()}
...
def visualize_results(rewards, success_rates, env, original_p, original_q, original_v, original_delta,
                      attacked_p, attacked_q, attacked_v, attacked_delta):
    """Visualize training results and power system parameters"""
...
def main():
    """Main function to run the training on predicted P values"""
    # Sample predicted P values (replace with your actual values)
    # This represents the power values that you want to attack
    predicted_p_values = np.array([10.0, 12.5, 8.0, 15.2, 9.8])

    # Create environment with these values
    # You can specify which indices to target, or leave as None to target all
    target_indices = [0, 1, 2, 3, 4]  # Target all values
    env = PredictedPAttackEnv(predicted_p_values, target_indices=target_indices, episode_length=50)
    ...
    # Train agent
    rewards, success_rates = train_agent(env, agent, episodes=120, max_steps=50, print_interval=20)

    # Evaluate agent
    eval_rewards, eval_success = evaluate_agent(env, agent)

    # Calculate Q, V, and delta for original P values
    original_Q, original_V, original_delta = calculate_q_v_delta_bfs_simplified(predicted_p_values)

    # Calculate Q, V, and delta for attacked P values
